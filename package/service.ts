...
import { Options, routesMap } from "./main";
        ...
        (e: WatchEventType, filename: string) => {
            if (e === 'rename') {
                ...
                const mapKey = addPrefSlash(
                    filename
                        .replaceAll('\\', '/')
                        .replace(`/${mergedOptions.templateName}`, ''),
                );
                if (
                    fs.existsSync(filePath) &&
                    filename.endsWith(mergedOptions.templateName)
                ) {
                    console.log(mapKey);
...
export function redirect(server: ViteDevServer, mergedOptions: { templateName: string; dir: string | PathLike }) {
